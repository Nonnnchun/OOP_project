...
def get():
    return Container(
        # หัวข้อหลัก
        H1(f"Hello {user_firstname} {user_lastname}", style="text-align: center; color: #FFFFFF; margin: 20px 0;"),
        H6(f"Total point : {user_points}", style="text-align: center; color: #FFFFFF; margin: 20px 0;"),
        
        Grid(
            # Card 1 พาเช้าไปหน้า edit profile
            Card(
                H3("Edit profile", style="color: #ffee63;"),
                P("Edit my personal detail", 
...
def password_change(request):
    if request.method == "POST":
        ...
        # Fetching account details to check
        account = controller.account_list[0]  # Assuming one account for simplicity
        if old_password != account.password:
            return Container(H3("Old password is incorrect", style="color: red;"))
        if new_password != confirm_new_password:
            return Container(H3("New passwords do not match", style="color: red;"))
        
        # Update password
        account.password = new_password
        return Container(H3("Password changed successfully!", style="color: green;"))

    
    return Container(
        H2("Change Your Password", style="text-align: center; color: #fff; margin: 20px 0;"),
        Form(
            Input(name="old_password", type="password", placeholder="Enter old password", style="margin-bottom: 10px;"),
            Input(name="new_password", type="password", placeholder="Enter new password", style="margin-bottom: 10px;"),
            Input(name="confirm_new_password", type="password", placeholder="Confirm new password", style="margin-bottom: 10px;"),
